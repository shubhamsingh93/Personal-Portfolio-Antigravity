        ...
        >
          <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full">
            ...
            <div className="flex flex-col gap-2 group">
              <label htmlFor="name" className="text-xs uppercase tracking-widest text-muted group-focus-within:text-cyan transition-colors font-bold">Name</label>
              <input 
                id="name"
                name="name"
                type="text" 
                ...
                className="w-full bg-transparent border-b border-white/10 py-3 text-offwhite placeholder:text-white/20 focus:outline-none focus:border-cyan transition-colors"
              />
            </div>

            <div className="flex flex-col gap-2 group">
              <label htmlFor="email" className="text-xs uppercase tracking-widest text-muted group-focus-within:text-cyan transition-colors font-bold">Email</label>
              <input 
                id="email"
                name="email"
                type="email" 
            ...
            <div className="flex flex-col gap-2 group mt-4">
              <label htmlFor="message" className="text-xs uppercase tracking-widest text-muted group-focus-within:text-cyan transition-colors font-bold">Message</label>
              <textarea 
                id="message"
                name="message"
                required
                ...
                rows={4}
                className="w-full bg-transparent border-b border-white/10 py-3 text-offwhite placeholder:text-white/20 focus:outline-none focus:border-cyan transition-colors resize-none"
              />
            </div>

            {/* Status Messages */}
            {status === "error" && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-red-400 text-sm mt-2 p-3 bg-red-400/10 rounded-lg border border-red-400/20">
            ...
            {status === "success" && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2 text-green-400 text-sm mt-2 p-3 bg-green-400/10 rounded-lg border border-green-400/20">
            ...
            <button 
              ...
              className={clsx(
                "mt-6 w-full py-4 rounded-xl font-bold uppercase tracking-widest text-sm flex items-center justify-center gap-3 transition-all duration-300",
            ...
            >
              {status === "idle" && <><Send className="w-4 h-4" /> Secure Dispatch</>}
